...
public class DynamoDBTableCreator {
    ...
    public DynamoDBTableCreator() {
        ...
        this.dynamoDbClient = DynamoDbClient.builder().credentialsProvider(StaticCredentialsProvider.create(awsCreds)).region(Region.US_EAST_1).build();
    }
    ...
    private void createTableIfNotExists(String tableName, String primaryKey) {
        try {
            ...
            CreateTableRequest createTableRequest = CreateTableRequest.builder().tableName(tableName).keySchema(KeySchemaElement.builder().attributeName(primaryKey).keyType(KeyType.HASH) .build()).attributeDefinitions(AttributeDefinition.builder().attributeName(primaryKey).attributeType(ScalarAttributeType.S) .build()).provisionedThroughput(ProvisionedThroughput.builder().readCapacityUnits(5L)
                .writeCapacityUnits(5L).build()).build();
    ...
    private void createCourseEnrollmentTableIfNotExists() {
        ...
        try {
            ...
            CreateTableRequest createTableRequest = CreateTableRequest.builder()
                    ...
                    .attributeDefinitions(AttributeDefinition.builder().attributeName("courseId").attributeType(ScalarAttributeType.S) .build(),
